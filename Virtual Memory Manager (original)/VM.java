...
public class VM
{
	...
	/* Creates page for virtual memory system. */
	...
	/* Gets next available Frame Number. */
   
	public int getNextFrame() {
		return nextFrameNumber++;
	}

	/* Checks for mapping in TLB for page number. */
   
	public int checkTLB(int pageNumber) {
		int frameNumber = -1;

		/* If mapping is found for the page number, gets Frame no. and increments TLB hits. */
	...
	public void setTLBMapping(int pageNumber, int frameNumber) {
		...
		/* After mapping has been set, increment the next entry index for TLB. */
	...
	public int getPhysicalAddress(int virtualAddress) throws java.io.IOException {
		...
		/* Checks for Frame no. in TLB. If not matched, then will check page table. */
		...
		if ( (frameNumber = checkTLB(pageNumber)) == -1 ) {  /** TLB Miss **/
			...
			if (pageTable[pageNumber].getValidBit() == true) {
				...
				/* Set up to check for page faults, reads content of page and stores in new frame.
                                   Then sets mapping in page table. */
	...
	public byte getValue(int physicalAddress) throws java.io.IOException {
		...
		/* Reads specified words from specified frames in relation to physical address. */
